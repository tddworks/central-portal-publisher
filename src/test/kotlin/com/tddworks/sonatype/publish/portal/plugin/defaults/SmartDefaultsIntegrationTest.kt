 ...
 *
 * Verifies that smart defaults are properly integrated into the configuration precedence chain and
 * work correctly with other configuration sources.
 */
class SmartDefaultsIntegrationTest {
    ...
    fun `should apply smart defaults when no other configuration is provided`() {
        ...
        val config =
            configManager.loadConfigurationWithPrecedence(
                dslConfig = null,
                propertiesFile = null,
        ...
        assertThat(config.projectInfo.license.name)
            .isEqualTo("Apache License 2.0") // From GenericProjectDefaultProvider
    ...
    fun `should provide smart defaults for project name inference`() {
        ...
        val namedProject =
            ProjectBuilder.builder().withName("my-awesome-library").withProjectDir(tempDir).build()
        val namedConfigManager = ConfigurationSourceManager(namedProject)

        // When
        val config =
            namedConfigManager.loadConfigurationWithPrecedence(
                dslConfig = null,
                enableAutoDetection = true,
            )
    ...
    fun `should handle multi-module project naming with smart defaults`() {
        ...
        val rootProject = ProjectBuilder.builder().withName("parent-project").build()
        ...
        // When
        val config =
            subConfigManager.loadConfigurationWithPrecedence(
                dslConfig = null,
                enableAutoDetection = true,
            )
