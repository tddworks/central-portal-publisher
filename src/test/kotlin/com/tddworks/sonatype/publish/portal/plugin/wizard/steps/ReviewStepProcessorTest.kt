...
class ReviewStepProcessorTest {
    ...
    fun `should display configuration summary and allow user to proceed`() {
        ...
        whenever(
                mockPromptSystem.confirm(
                    "Does this configuration look correct? Proceed with setup?"
                )
            )
            .thenReturn(true)
    ...
    fun `should allow user to go back and make changes`() {
        ...
        assertThat(result.validationErrors)
            .contains("Please make necessary changes and return to review")
    }
    ...
    fun `should show auto-detection status in review`() {
        // Given
        val context =
            createTestContextWithFullConfig()
                .copy(hasAutoDetectedCredentials = true, hasAutoDetectedSigning = true)
        whenever(
                mockPromptSystem.confirm(
                    "Does this configuration look correct? Proceed with setup?"
                )
    ...
    fun `should show manual configuration status in review`() {
        ...
        val context =
            createTestContextWithFullConfig()
                .copy(hasAutoDetectedCredentials = false, hasAutoDetectedSigning = false)
        whenever(
                mockPromptSystem.confirm(
                    "Does this configuration look correct? Proceed with setup?"
                )
    ...
    fun `should display review summary without waiting for input before confirmation`() {
        ...
        verify(mockPromptSystem)
            .display(argThat { message -> message.contains("📋 CONFIGURATION REVIEW") })
        verify(mockPromptSystem)
            .confirm("Does this configuration look correct? Proceed with setup?")
    }
    ...
    fun `should show progress indicator in review summary`() {
        ...
        verify(mockPromptSystem)
            .display(
                argThat { message -> message.contains("📋 CONFIGURATION REVIEW (Step 5 of 6)") }
            )
    }
    ...
    fun `should display security status correctly for configured credentials and signing`() {
        ...
        val context =
            createTestContextWithFullConfig()
                .copy(
                    ...
                    wizardConfig =
                        TestConfigBuilder.createConfig()
                            .copy(
                                credentials =
                                    TestConfigBuilder.createConfig()
                                        .credentials
                                        .copy(username = "test-user", password = "test-pass"),
                                signing =
                                    TestConfigBuilder.createConfig()
                                        .signing
                                        .copy(keyId = "test-key", password = "test-key-pass"),
                            ),
                )
        whenever(
                mockPromptSystem.confirm(
        ...
        verify(mockPromptSystem)
            .display(
                argThat { message ->
                    message.contains("✅ Credentials: Configured") &&
                        message.contains("✅ Signing: Configured")
                }
            )
    }
    ...
    private fun createTestContextWithFullConfig() =
        WizardContext(
