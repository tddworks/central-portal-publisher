...
class MavenPublicationUtilsTest {
        ...
        project.extensions.configure(
            ...
            object : Action<PublishingExtension> {
                override fun execute(publishing: PublishingExtension) {
                    publication =
                        publishing.publications.create("test", MavenPublication::class.java)
                }
            },
        )
    }
    ...
    fun `should configure POM with license information`() {
        ...
        // in unit tests due to Gradle's lazy evaluation, but we verify the configuration doesn't
        // throw)
    ...
    fun `should use project name when config name is blank`() {
        ...
        testProject.extensions.configure(
            PublishingExtension::class.java,
            object : Action<PublishingExtension> {
                override fun execute(publishing: PublishingExtension) {
                    testPublication =
    ...
    fun `should configure signing with keyId only`() {
        ...
        val signingConfig =
            CentralPublisherConfig(
                projectInfo = config.projectInfo,
                credentials = config.credentials,
                signing = SigningConfig(keyId = "test-key-id", password = "test-password"),
            )
