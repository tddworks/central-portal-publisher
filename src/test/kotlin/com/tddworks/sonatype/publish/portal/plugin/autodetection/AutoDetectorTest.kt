...
class AutoDetectorTest {
    ...
    fun `should create detection result with detected values`() {
        ...
        val detectedValues = mapOf(
            "projectInfo.name" to DetectedValue(
                path = "projectInfo.name",
                value = "test-project",
                source = "build.gradle.kts",
                confidence = Confidence.HIGH
            ),
            "projectInfo.description" to DetectedValue(
    ...
    fun `auto detection manager should merge multiple detector results`() {
        // Given
        val detector1 = MockDetector(
            "detector1",
            CentralPublisherConfigBuilder()
                .projectInfo { name = "detected-name" }
                .build(),
            mapOf(
                "projectInfo.name" to DetectedValue(
                    "projectInfo.name", 
    ...
    fun `auto detection manager should prioritize higher confidence values`() {
        ...
        val lowConfidenceDetector = MockDetector(
            "low-detector",
            CentralPublisherConfigBuilder()
        ...
        val highConfidenceDetector = MockDetector(
            "high-detector",
            CentralPublisherConfigBuilder()
        ...
        // Then
        assertThat(summary.detectedValues["projectInfo.name"]?.value).isEqualTo("high-confidence-name")
        assertThat(summary.detectedValues["projectInfo.name"]?.confidence).isEqualTo(Confidence.HIGH)
    }
    
    @Test
    fun `auto detection manager should handle detector failures gracefully`() {
        // Given
        val failingDetector = object : AutoDetector {
        ...
        val workingDetector = MockDetector(
            "working-detector",
            CentralPublisherConfigBuilder().projectInfo { name = "working" }.build(),
            mapOf("projectInfo.name" to DetectedValue("projectInfo.name", "working", "test", Confidence.HIGH))
        )
        ...
        assertThat(summary.warnings).contains("Detector 'failing-detector' failed: Simulated detector failure")
    ...
    fun `auto detection summary should filter by confidence level`() {
        ...
        val summary = AutoDetectionSummary(
