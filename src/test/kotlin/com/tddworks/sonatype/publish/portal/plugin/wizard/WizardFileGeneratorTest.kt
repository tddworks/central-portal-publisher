...
class WizardFileGeneratorTest {
    ...
    fun `should generate both gradle properties and build file when neither credentials nor signing are auto-detected`() {
        ...
        val context =
            WizardContext(
                project = project,
                detectedInfo =
                    DetectedProjectInfo("test-project", "https://github.com/test/test-project"),
                    ...
                    projectInfo =
                        TestConfigBuilder.createConfig()
                            ...
                            .copy(
                                ...
                                url =
                                    "", // Empty so auto-detected "https://github.com/auto/detected"
                                // is
                                // used
                                developers =
                                    emptyList(), // Empty so auto-detected developers are used
                            )
                )
    ...
    fun `should generate dynamic file list based on what was actually created`() {
        // Given - Both credentials and signing are auto-detected (no gradle.properties should be
        // generated)
        val project =
            ProjectBuilder.builder()
                .withProjectDir(tempDir.toFile())
    ...
    fun `should update existing centralPublisher block without duplicating`() {
                    ...
                    projectInfo =
                        TestConfigBuilder.createConfig()
                            ...
                            .copy(
                                ...
                                url =
                                    "", // Empty so auto-detected "https://github.com/new/project" is
                                // used
                            )
                )
        ...
        assertThat(updatedContent.split("centralPublisher \\{".toRegex()))
            .hasSize(2) // Split creates 2 parts for 1 occurrence
    }
