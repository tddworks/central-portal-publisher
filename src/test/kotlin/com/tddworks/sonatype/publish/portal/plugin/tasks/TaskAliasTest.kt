...
class TaskAliasTest {
    ...
    fun `should provide correct task alias mappings`() {
        ...
        // Main task mappings
        assertThat(SimplifiedTaskRegistry.getNewTaskName("publishAllPublicationsToSonatypePortalRepository"))
            .isEqualTo("publishToCentral")
        assertThat(SimplifiedTaskRegistry.getNewTaskName("publishAggregationPublicationsToSonatypePortalRepository"))
            .isEqualTo("publishToCentral")
        ...
        assertThat(SimplifiedTaskRegistry.getNewTaskName("publishMavenPublicationToSonatypePortalRepository"))
            .isEqualTo("publishMaven")
        assertThat(SimplifiedTaskRegistry.getNewTaskName("publishKotlinMultiplatformPublicationToSonatypePortalRepository"))
    ...
    fun `should return original task name when no alias exists`() {
        ...
        assertThat(SimplifiedTaskRegistry.getNewTaskName("build"))
            .isEqualTo("build")
    }
    ...
    fun `should have consistent naming convention`() {
        ...
        publishTasks.forEach { taskName ->
            assertThat(taskName).startsWith("publish")
        }
        
        // All bundle tasks should start with 'bundle'  
        val bundleTasks = newTaskNames.filter { it.contains("bundle", ignoreCase = true) }
        bundleTasks.forEach { taskName ->
            assertThat(taskName).startsWith("bundle")
        }
        
        // Task names should be camelCase
        newTaskNames.forEach { taskName ->
            assertThat(taskName).matches("[a-z][a-zA-Z]*")
        }
    }
