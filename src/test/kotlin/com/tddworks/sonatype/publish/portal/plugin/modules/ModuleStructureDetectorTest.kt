...
/**
 * Tests for ModuleStructureDetector that identifies multi-module Gradle project structures and
 * provides configuration inheritance capabilities.
 */
class ModuleStructureDetectorTest {
    ...
    fun `should detect multi-module project with settings gradle`() {
        ...
        assertThat(moduleStructure.subModules.map { it.name })
            .containsExactlyInAnyOrder("core", "api", "cli")

        // Verify detected values
        assertThat(result!!.detectedValues).containsKey("modules.structure")
        assertThat(result.detectedValues["modules.structure"]?.confidence)
    ...
    fun `should handle nested module structures`() {
        ...
        assertThat(moduleStructure.subModules.map { it.path })
    ...
    fun `should detect module dependencies`() {
        ...
        File(File(tempDir, "core").apply { mkdirs() }, "build.gradle.kts")
            .writeText(
                """
            plugins { kotlin("jvm") }
        """
        ...
        File(File(tempDir, "api").apply { mkdirs() }, "build.gradle.kts")
            .writeText(
                """
            plugins { kotlin("jvm") }
            dependencies {
    ...
    fun `should provide module configuration inheritance capabilities`() {
        ...
        assertThat(moduleStructure.inheritanceStrategy)
            .isEqualTo(ModuleInheritanceStrategy.ROOT_OVERRIDES)
    }

    @Test
    fun `should handle settings gradle groovy format`() {
        // Given - Groovy settings.gradle
        File(tempDir, "settings.gradle")
            .writeText(
                """
    ...
    fun `should detect publishing modules only`() {
        ...
        assertThat(moduleStructure.publishableModules.map { it.name })
            .containsExactlyInAnyOrder("core", "api")
        assertThat(moduleStructure.nonPublishableModules).hasSize(1)
        assertThat(moduleStructure.nonPublishableModules.map { it.name })
            .containsExactly("test-utils")
    }
}
