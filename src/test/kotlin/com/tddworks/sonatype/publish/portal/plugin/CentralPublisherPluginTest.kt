...
class CentralPublisherPluginTest {
    ...
    fun `should set correct task group and descriptions`() {
        ...
        assertThat(publishTask.description)
            .isEqualTo("🚀 Publish your artifacts to Maven Central (creates bundle and uploads)")
        ...
        assertThat(setupTask.description)
            .contains("Set up your project for Maven Central publishing")
    }
    ...
    fun `should provide clean task names compared to old plugin`() {
        ...
        val centralTaskNames =
            project.tasks.names.filter { taskName ->
                val task = project.tasks.findByName(taskName)
                task?.group == "Central Publishing"
            }
        assertThat(centralTaskNames)
            .hasSize(4) // publishToCentral, bundleArtifacts, validatePublishing, setupPublishing
    }
    ...
    fun `should handle auto-detection and validation`() {
        ...
        val validationEngine =
            com.tddworks.sonatype.publish.portal.plugin.validation.ValidationEngine()
    ...
    fun `should not show validation errors when no explicit configuration exists`() {
        ...
        // Note: Logger output goes to logger.quiet() which may not be captured by System.out
        // redirection
        // The core behavior is that hasExplicitConfiguration returns false when no DSL
        // configuration is
        // used
    }
    ...
    fun `should show validation errors when explicit configuration exists but is invalid`() {
        ...
        // Note: Validation error messages are logged via logger.error() and may not be captured in
        // System.out
    ...
    fun `should pass validation when explicit configuration exists and is valid`() {
        ...
        // The important behavior is that hasExplicitConfiguration() correctly identifies explicit
        // config
    }
    ...
    fun `should handle hasExplicitConfiguration correctly across different configurations`() {
        ...
        extension.credentials { username = "test-user" }
        assertThat(extension.hasExplicitConfiguration()).isTrue()

        // When - Configure another section
        extension.projectInfo { name = "test-project" }
        assertThat(extension.hasExplicitConfiguration()).isTrue()
    }
}
