...
class ProjectInfoStepProcessorTest {
    ...
    fun `should prompt for manual input when user rejects auto-detected values`() {
        ...
        assertThat(updatedWizardConfig.projectInfo.description)
            .isEqualTo("A manually configured project")
        assertThat(updatedWizardConfig.projectInfo.url)
            .isEqualTo("https://github.com/manual/project")
        assertThat(updatedWizardConfig.projectInfo.developers).hasSize(1)
        assertThat(updatedWizardConfig.projectInfo.developers.first().name)
            .isEqualTo("Manual Developer")
        assertThat(updatedWizardConfig.projectInfo.developers.first().email)
            .isEqualTo("manual@dev.com")
    }
    ...
    fun `should ask about each detected field individually and allow selective acceptance`() {
        ...
        val mockPromptSystem =
            MockPromptSystem().apply {
        ...
        assertThat(config.projectInfo.url)
            .isEqualTo("https://github.com/manual/url") // manually entered
        assertThat(config.projectInfo.description)
            .isEqualTo("Manual project description") // manually entered
        assertThat(config.projectInfo.developers.first().name).isEqualTo("Auto User") // accepted
        assertThat(config.projectInfo.developers.first().email)
            .isEqualTo("auto@example.com") // accepted

        // Verify prompts asked about each field
        assertThat(mockPromptSystem.allPrompts)
            .contains("Use auto-detected project name 'auto-detected-project'?")
        assertThat(mockPromptSystem.allPrompts)
            .contains("Use auto-detected project URL 'https://github.com/auto/detected'?")
        assertThat(mockPromptSystem.allPrompts)
            .contains("Use auto-detected developer 'Auto User <auto@example.com>'?")
    }
}
