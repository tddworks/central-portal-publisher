...
/**
 * Tests for Selective Publishing functionality that allows fine-grained control 
 * over which modules get published in multi-module projects.
 * 
 * This includes include/exclude patterns, module filters, selection DSL, 
 * module groups, and validation.
 */
class SelectivePublishingTest {
    ...
    fun `should support include patterns for module selection`() {
        ...
        val selectionConfig = ModuleSelectionConfig(
            includePatterns = listOf("core", "api", "ui")
        )
    ...
    fun `should support exclude patterns for module selection`() {
        ...
        val moduleStructure = moduleStructureDetector.analyzeModuleStructure(tempDir)
        val selectionConfig = ModuleSelectionConfig(
            excludePatterns = listOf("*test*", "benchmarks")
        )
    ...
    fun `should combine include and exclude patterns correctly`() {
        // Given - Multi-module project
        createMultiModuleProject(
            modules = listOf("frontend-core", "frontend-ui", "backend-core", "backend-api", "shared-utils"),
            publishableModules = listOf("frontend-core", "frontend-ui", "backend-core", "backend-api", "shared-utils")
        )
        
        val moduleStructure = moduleStructureDetector.analyzeModuleStructure(tempDir)
        val selectionConfig = ModuleSelectionConfig(
            includePatterns = listOf("frontend-*", "backend-*"),  // Include frontend and backend modules
        ...
        // Then
        assertThat(selectedModules.map { it.name }).containsExactlyInAnyOrder("frontend-core", "backend-core", "backend-api")
    }
    ...
    fun `should support module filters by dependency presence`() {
        ...
        val selectionConfig = ModuleSelectionConfig(
            filters = listOf(
                ModuleFilter.HasDependencies(hasProjectDependencies = true)
            )
        )
    ...
    fun `should support module groups for logical organization`() {
        ...
        createMultiModuleProject(
            modules = listOf("frontend-core", "frontend-ui", "backend-core", "backend-api", "shared-common"),
            publishableModules = listOf("frontend-core", "frontend-ui", "backend-core", "backend-api", "shared-common")
        )
        ...
        // Then
        assertThat(selectedModules.map { it.name }).containsExactlyInAnyOrder("backend-core", "backend-api", "shared-common")
    }
    ...
    fun `should validate module selection configuration`() {
        ...
        val moduleStructure = moduleStructureDetector.analyzeModuleStructure(tempDir)
        val invalidSelectionConfig = ModuleSelectionConfig(
            includePatterns = listOf("nonexistent-module"),
            selectedGroups = listOf("invalid-group")
        )
        
        // When
        val validationResult = selectivePublisher.validateSelection(moduleStructure, invalidSelectionConfig)
    ...
    fun `should support complex selection with multiple criteria`() {
        ...
        val selectionConfig = ModuleSelectionConfig(
            ...
            filters = listOf(
                ModuleFilter.PublishableOnly(true),
                ModuleFilter.HasDependencies(hasProjectDependencies = false) // Leaf modules only
            ),
            groups = mapOf(
                "core-libs" to listOf("frontend-core", "backend-core")
            ),
        ...
        assertThat(selectedModules.map { it.name }).containsExactlyInAnyOrder("frontend-core", "backend-core")
        assertThat(selectedModules).allSatisfy { module ->
            assertThat(module.isPublishable).isTrue()
    ...
    fun `should support wildcard patterns in include and exclude`() {
        ...
        createMultiModuleProject(
            modules = listOf("lib-core", "lib-api", "lib-ui", "app-main", "test-integration", "test-unit"),
        ...
        // Then
        assertThat(selectedModules.map { it.name }).containsExactlyInAnyOrder("lib-core", "lib-api", "app-main")
    }
    ...
    private fun createMultiModuleProject(
        ...
        modules.forEach { moduleName ->
            ...
            val buildFileContent = if (isPublishable) {
                """
                plugins {
                    kotlin("jvm")
                    `maven-publish`
    ...
    private fun createMultiModuleProjectWithDependencies() {
        ...
        File(cliDir, "build.gradle.kts").writeText("""
            plugins {
                kotlin("jvm")
                `maven-publish`
            }
    ...
    private fun createComplexMultiModuleProject() {
        ...
        File(frontendCoreDir, "build.gradle.kts").writeText("""
        ...
        File(frontendUiDir, "build.gradle.kts").writeText("""
            plugins { kotlin("jvm"); `maven-publish` }
