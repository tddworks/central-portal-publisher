...
class ProjectInfoDetectorTest {
    ...
    fun `should detect project name from gradle project`() {
        // Given
        val project =
            ProjectBuilder.builder().withName("my-awesome-project").withProjectDir(tempDir).build()
    ...
    fun `should detect project name from directory when gradle name is root`() {
        ...
        assertThat(result.detectedValues["projectInfo.name"]?.confidence)
    ...
    fun `should detect description from build gradle kts`() {
        ...
        // Then
        assertThat(result).isNotNull
        assertThat(result!!.config.projectInfo.description)
            .isEqualTo("A fantastic Kotlin library for doing amazing things")
        assertThat(result.detectedValues["projectInfo.description"]?.confidence)
            .isEqualTo(Confidence.HIGH)
        assertThat(result.detectedValues["projectInfo.description"]?.source)
            .isEqualTo("build.gradle.kts")
    }
    ...
    fun `should detect description from groovy build gradle`() {
        ...
        assertThat(result).isNotNull
        assertThat(result!!.config.projectInfo.description)
            .isEqualTo("A Java library with Groovy build script")
        assertThat(result.detectedValues["projectInfo.description"]?.confidence)
            .isEqualTo(Confidence.HIGH)
        assertThat(result.detectedValues["projectInfo.description"]?.source)
    ...
    fun `should handle README with badges and links`() {
        ...
        assertThat(result).isNotNull
        assertThat(result!!.config.projectInfo.description)
            .isEqualTo("This library provides essential functionality for modern applications.")
        assertThat(result.detectedValues["projectInfo.description"]?.source).isEqualTo("README.md")
    }
    ...
    fun `should add warnings for missing information`() {
        ...
        assertThat(result!!.warnings)
            .hasSize(1) // Only description warning, name is detected from directory
        assertThat(result.warnings)
            .contains(
                "Could not auto-detect project description from build.gradle.kts or README files"
            )
    ...
    fun `should return null when no information can be detected`() {
        ...
        val tempProject =
            File.createTempFile("junit", "test").let { file ->
                file.delete()
                file.mkdirs()
                file
            }

        val project = ProjectBuilder.builder().withName("root").withProjectDir(tempProject).build()
