 ...
 *
 * The DryRunEngine should validate configuration and simulate all publishing steps without actually
 * performing any network operations or file uploads.
 */
class DryRunEngineTest {
    ...
    fun `should simulate all publishing steps without side effects`() {
        ...
        assertThat(stepNames)
            .containsAnyOf("Aggregated Upload Simulation", "Individual Upload Simulation")
    ...
    fun `should simulate different outcomes based on configuration`() {
        ...
        val autoPublishConfig =
            createValidConfig().copy(publishing = PublishingConfig(autoPublish = true))
        val manualConfig =
            createValidConfig().copy(publishing = PublishingConfig(autoPublish = false))
    ...
    fun `should handle aggregation vs individual publishing simulation`() {
        ...
        val aggregatedConfig =
            createValidConfig().copy(publishing = PublishingConfig(aggregation = true))
        val individualConfig =
            createValidConfig().copy(publishing = PublishingConfig(aggregation = false))
    ...
    fun `should not perform actual operations when dry run is disabled but dryRun config is true`() {
        ...
        val config = createValidConfig().copy(publishing = PublishingConfig(dryRun = true))
        ...
        return CentralPublisherConfig(
            ...
            projectInfo =
                ProjectInfoConfig(
                    ...
                    developers =
                        listOf(DeveloperConfig(name = "Test Developer", email = "test@example.com")),
                ),
            signing =
                SigningConfig(
                    keyId = "12345678",
            ...
            publishing = PublishingConfig(autoPublish = false, aggregation = true, dryRun = false),
        )
    }
}
