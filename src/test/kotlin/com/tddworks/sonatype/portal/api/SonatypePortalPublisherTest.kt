...
class `Sonatype Portal Publisher` {
    ...
    inner class `When deployment is valid` {
        ...
        fun `should configure uploader with correct authentication`() {
            ...
            assertThat(capturedBuilder.getHeaders()).containsEntry("Authorization", "UserToken dGVzdC11c2VyOnRlc3QtcGFzc3dvcmQ=")
        }
    ...
    inner class `When using result-based deployment` {
        ...
        fun `should return failure result when upload fails`() {
            ...
            assertThat(result.isFailure()).isTrue()
            assertThat(result.getErrorMessageOrNull()).contains("Failed to upload deployment bundle")
        }
    }
...
/**
 * Test double for FileUploader following Chicago School principles.
 * Uses real state instead of mocked interactions.
 */
