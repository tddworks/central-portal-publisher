...
class WelcomeStepProcessor : WizardStepProcessor {
    ...
    override fun process(context: WizardContext, promptSystem: PromptSystem): WizardStepResult {
        ...
        val welcomeMessage = buildString {
            appendLine(
                "🧙 WELCOME TO CENTRAL PUBLISHER SETUP (Step $currentStepIndex of $totalSteps)"
            )
            ...
            context.detectedInfo?.let { detected ->
                ...
                if (detected.developers.isNotEmpty()) {
                    appendLine(
                        "• Developers: ${detected.developers.joinToString { "${it.name} <${it.email}>" }}"
                    )
                }
                appendLine()
            }
        ...
        return WizardStepResult(currentStep = step, isValid = true, validationErrors = emptyList())
    }
}
