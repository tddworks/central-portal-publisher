...
class BundleArtifactsTaskExecutor(
    ...
    fun execute() {
        ...
        try {
            ...
            val publishingExtension =
                project.extensions.findByType(PublishingExtension::class.java)
                    ?: throw GradleException(
                        "maven-publish plugin not applied. Please apply 'maven-publish' plugin."
                    )
            ...
            project.logger.quiet(
                "💡 Next step: Run './gradlew publishToCentral' to upload to Maven Central"
            )
        } catch (e: Exception) {
    ...
    /**
     * Finds local repository directory with artifacts. In multi-module projects, all artifacts are
     * published to the root project's repository.
     */
    private fun findLocalRepositoryWithArtifacts(): File? {
        ...
        project.logger.quiet(
            "🔍 Looking for artifacts in root project repository: ${signedRepoDir.absolutePath}"
        )
        ...
        when {
            ...
            unsignedRepoDir.exists() && unsignedRepoDir.listFiles()?.isNotEmpty() == true -> {
                ...
                project.logger.warn(
                    "💡 See SIGNING_SETUP.md for signing configuration instructions"
                )
                ...
                project.logger.quiet(
                    "   - ${signedRepoDir.absolutePath} (exists: ${signedRepoDir.exists()})"
                )
                project.logger.quiet(
                    "   - ${unsignedRepoDir.absolutePath} (exists: ${unsignedRepoDir.exists()})"
                )
                return null
            }
        }
