 ...
 * Essential for Maven Central publishing requirements.
 */
class RequiredFieldValidator : ConfigurationValidator {
    
    override val name = "RequiredFieldValidator"
    override val description = "Validates that required fields for Maven Central publishing are present"
    ...
    override fun validate(config: CentralPublisherConfig): List<ValidationViolation> {
        ...
        } else if (config.credentials.password == "password" || config.credentials.password.length < 8) {
