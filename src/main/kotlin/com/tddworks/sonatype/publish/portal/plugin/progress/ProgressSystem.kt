...
/**
 * Progress tracking system for long-running publishing operations.
 * Provides visual progress bars, time estimates, and detailed feedback.
 */
...
class ProgressTracker(
    ...
    val hasFailed: Boolean get() = _hasFailed
    ...
    fun nextStep(message: String = "") {
        ...
        val progress = ProgressUtils.createProgressBar(currentStep, totalSteps, ProgressBarStyle.BLOCKS)
    ...
    fun complete(message: String = "Task completed successfully") {
        ...
        val progressBar = ProgressUtils.createProgressBar(totalSteps, totalSteps, ProgressBarStyle.BLOCKS)
...
class FileUploadTracker(
    ...
    val uploadedBytes: Long get() = _uploadedBytes
    val isCompleted: Boolean get() = _isCompleted
    ...
    fun updateProgress(bytesUploaded: Long) {
        ...
        val progress = ProgressUtils.createProgressBar(progressPercentage, 100, ProgressBarStyle.BLOCKS)
    ...
    fun complete(message: String = "Upload completed") {
        ...
        val totalTime = System.currentTimeMillis() - _startTime
        val avgSpeed = if (totalTime > 0) ProgressUtils.formatBytes((totalBytes * 1000) / totalTime) + "/s" else "N/A"
        
        logger.lifecycle("✅ $fileName: $message")
        logger.lifecycle("  ${ProgressUtils.formatBytes(totalBytes)} in ${ProgressUtils.formatDuration(totalTime)} (avg $avgSpeed)")
...
object ProgressUtils {
    ...
    fun createProgressBar(current: Int, total: Int, style: ProgressBarStyle = ProgressBarStyle.BLOCKS): String {
