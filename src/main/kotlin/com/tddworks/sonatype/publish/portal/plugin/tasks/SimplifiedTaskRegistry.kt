...
/**
 * Registry for simplified task names that map to the original verbose task names. This provides a
 * more user-friendly interface while maintaining backward compatibility.
 */
class SimplifiedTaskRegistry {
    ...
    companion object {
        ...
        private val TASK_MAPPINGS =
            mapOf(
                SonatypePortalPublisherPlugin
                    .PUBLISH_ALL_PUBLICATIONS_TO_SONATYPE_PORTAL_REPOSITORY to PUBLISH_TO_CENTRAL,
                SonatypePortalPublisherPlugin
                    .PUBLISH_AGGREGATION_PUBLICATIONS_TO_SONATYPE_PORTAL_REPOSITORY to
                    PUBLISH_TO_CENTRAL,
    ...
    /**
     * Registers simplified task names that delegate to the original tasks. This maintains backward
     * compatibility while providing a cleaner interface.
     */
    ...
    private fun registerPublishToCentralTask(project: Project) {
            ...
            doLast {
                ...
                val oldPublishTask =
                    project.tasks.findByName(
                        SonatypePortalPublisherPlugin
                            .PUBLISH_ALL_PUBLICATIONS_TO_SONATYPE_PORTAL_REPOSITORY
                    )
                val oldAggregateTask =
                    project.tasks.findByName(
                        SonatypePortalPublisherPlugin
                ...
                when {
                    oldAggregateTask != null ->
                        logger.lifecycle("Delegating to: ${oldAggregateTask.name}")
                    oldPublishTask != null ->
                        logger.lifecycle("Delegating to: ${oldPublishTask.name}")
                    else -> {
                        logger.lifecycle("Publishing to Maven Central...")
                        logger.lifecycle(
                            "Note: This task will delegate to the actual publishing implementation"
                        )
                    }
                }
            }
        ...
        if (project.state.executed) {
            ...
            project.afterEvaluate { wireDependenciesForPublishTask(this) }
        }
    }
    ...
    private fun registerBundleArtifactsTask(project: Project) {
        project.tasks.register(BUNDLE_ARTIFACTS, DefaultTask::class.java) {
            ...
            doLast {
                val oldZipTask =
                    project.tasks.findByName(SonatypePortalPublisherPlugin.ZIP_ALL_PUBLICATIONS)
                val oldAggregateZipTask =
                    project.tasks.findByName(
                        SonatypePortalPublisherPlugin.ZIP_AGGREGATION_PUBLICATIONS
                    )

                when {
                    oldAggregateZipTask != null ->
                        logger.lifecycle("Delegating to: ${oldAggregateZipTask.name}")
                    oldZipTask != null -> logger.lifecycle("Delegating to: ${oldZipTask.name}")
                    else -> {
                        logger.lifecycle("Creating deployment bundles...")
                        logger.lifecycle(
                            "Note: This task will delegate to the actual bundling implementation"
                        )
                    }
                }
            }
        ...
        if (project.state.executed) {
            ...
            // Project not yet evaluated, wire later
            project.afterEvaluate { wireDependenciesForBundleTask(this) }
        }
    }
    ...
    private fun wireDependenciesForBundleTask(project: Project) {
        ...
        val oldZipTask =
            project.tasks.findByName(SonatypePortalPublisherPlugin.ZIP_ALL_PUBLICATIONS)
        val oldAggregateZipTask =
            project.tasks.findByName(SonatypePortalPublisherPlugin.ZIP_AGGREGATION_PUBLICATIONS)
    ...
    private fun addDeprecationWarnings(project: Project) {
        if (project.state.executed) {
            ...
            project.afterEvaluate { addDeprecationWarningsImpl(this) }
        }
    }
...
/**
 * Task for validating publishing configuration before attempting to publish. Uses the structured
 * error system for consistent reporting.
 */
abstract class PublishingValidationTask : DefaultTask() {
    ...
    fun validate() {
        ...
        val context = ErrorContext(projectPath = project.path, taskName = name)

        val errors =
            mutableListOf<com.tddworks.sonatype.publish.portal.plugin.error.PublishingError>()
    ...
    private fun validateCredentials(
        ...
        errors: MutableList<com.tddworks.sonatype.publish.portal.plugin.error.PublishingError>,
    ) {
        val username =
    ...
    private fun validateProjectInfo(
        context: ErrorContext,
        errors: MutableList<com.tddworks.sonatype.publish.portal.plugin.error.PublishingError>,
    ) {
    ...
    private fun validateSigning(
        ...
        val signingKey =
            project.findProperty("signing.keyId") as? String ?: System.getenv("SIGNING_KEY")
        val signingPassword =
