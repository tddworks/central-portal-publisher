...
/**
 * Manages loading configuration from multiple sources with proper precedence.
 * Supports DSL, properties files, environment variables, and auto-detected values.
 */
class ConfigurationSourceManager(
     ...
     */
    private val defaultPropertyMappings = mapOf(
        // Credentials
     ...
     */
    private val environmentMappings = mapOf(
        "SONATYPE_USERNAME" to "credentials.username",
        "SONATYPE_PASSWORD" to "credentials.password",
        ...
        dslConfig?.let {
            if (hasNonEmptyValues(it)) {
                ...
                if (!it.credentials.username.isEmpty()) {
                    sourceDiagnostics.recordValue("credentials.username", it.credentials.username, ConfigurationSource.DSL)
                }
                if (!it.credentials.password.isEmpty()) {
                    sourceDiagnostics.recordValue("credentials.password", it.credentials.password, ConfigurationSource.DSL)
                }
    ...
    fun loadFromProperties(
        ...
        try {
            ...
            // Cache the result
            configurationCache[cacheKey] = CachedConfiguration(
                config = config,
            ...
            validationErrors.add(ConfigurationValidationError(
    ...
    fun loadFromEnvironment(): CentralPublisherConfig {
        ...
        if (envProperties.isNotEmpty()) {
            return applyPropertiesToBuilder(envProperties, environmentMappings, ConfigurationSource.ENVIRONMENT).build()
        }
     ...
     */
    private fun applyPropertiesToBuilder(properties: Map<String, String>, propertyMappings: Map<String, String>, source: ConfigurationSource = ConfigurationSource.PROPERTIES): CentralPublisherConfigBuilder {
        ...
        propertyMappings.forEach { (propKey, configPath) ->
            ...
            if (!value.isNullOrBlank()) {
                ...
                when {
                    ...
                    configPath.startsWith("projectInfo.") && parts.size == 2 -> projectProps[parts[1]] = value
                    ...
                    configPath.startsWith("projectInfo.developer.") -> developerProps[parts[2]] = value
        ...
        if (projectProps.isNotEmpty() || scmProps.isNotEmpty() || licenseProps.isNotEmpty() || developerProps.isNotEmpty()) {
            builder.projectInfo {
     ...
     */
    private fun applyConfigToBuilder(builder: CentralPublisherConfigBuilder, config: CentralPublisherConfig) {
    ...
    private fun validateConfiguration(config: CentralPublisherConfig) {
        try {
            ...
            val errorMessages = e.message?.split("\n")?.drop(1) ?: emptyList() // Skip first line "Configuration validation failed:"
            errorMessages.forEach { errorMessage ->
                if (errorMessage.startsWith("- ")) {
                    validationErrors.add(ConfigurationValidationError(
                        source = ConfigurationSource.PROPERTIES,
                        message = errorMessage.substring(2), // Remove "- " prefix
                        exception = e
                    ))
                }
            }
...
class ConfigurationSourceDiagnostics {
    ...
    private val _sourceValues = mutableMapOf<String, MutableList<Pair<String, ConfigurationSource>>>()
    
    val configurationSources: Set<ConfigurationSource> get() = _configurationSources
    
    val precedenceOrder = listOf(
    ...
    fun finalValue(configPath: String): String? {
        return sourceValues[configPath]?.sortedBy { (_, source) -> 
            precedenceOrder.indexOf(source) 
        }?.firstOrNull()?.first
    }
