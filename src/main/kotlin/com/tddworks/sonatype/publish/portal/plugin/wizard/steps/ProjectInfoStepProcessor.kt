...
class ProjectInfoStepProcessor : WizardStepProcessor {
    ...
    private fun handleIndividualFieldConfirmation(
        ...
        promptSystem.display(
            "📝 PROJECT INFO (Step $currentStepIndex of $totalSteps)\n${"=".repeat(50)}\n"
        )
        ...
        // Project Name
        val useProjectName =
            if (detectedInfo.projectName.isNotBlank()) {
                promptSystem.confirm(
                    "Use auto-detected project name '${detectedInfo.projectName}'?"
                )
            } else false
        ...
        // Project URL
        val useProjectUrl =
            if (detectedInfo.projectUrl.isNotBlank()) {
        ...
        // Developer
        val useDeveloper =
            if (detectedInfo.developers.isNotEmpty()) {
                val firstDev = detectedInfo.developers.first()
                promptSystem.confirm(
                    "Use auto-detected developer '${firstDev.name} <${firstDev.email}>'?"
                )
            } else false
    ...
    private fun handleManualInput(
        ...
        // License information
        val licenseName =
            promptSystem.prompt("Enter license name (e.g., Apache License 2.0):")?.trim() ?: ""
        ...
        val updatedConfig =
            context.wizardConfig.copy(
                projectInfo =
                    context.wizardConfig.projectInfo.copy(
                        name = projectName,
                        description = projectDescription,
                        url = projectUrl,
                        license =
                            context.wizardConfig.projectInfo.license.copy(
                                name = licenseName,
                                url = licenseUrl,
                            ),
                        developers =
                            listOf(
                                com.tddworks.sonatype.publish.portal.plugin.config.DeveloperConfig(
                                    id =
                                        developerId.ifEmpty { developerEmail.substringBefore("@") },
                                    name = developerName,
                                    email = developerEmail,
                                )
                            ),
