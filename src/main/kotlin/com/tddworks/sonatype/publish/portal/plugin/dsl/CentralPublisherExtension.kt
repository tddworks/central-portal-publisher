...
/**
 * DSL marker annotation to prevent scope pollution and provide better IDE support. This ensures
 * that DSL blocks cannot access methods from outer scopes accidentally.
 */
@DslMarker
 ...
 *
 * This extension enables zero-configuration publishing to Maven Central with automatic detection of
 * project information from git repositories and build files.
 *
...
open class CentralPublisherExtension(private val project: Project) {
     ...
     *
     * This block supports auto-detection from git repositories and build files. Explicitly
     * configured values take precedence over auto-detected ones.
     *
        ...
        configBuilder.projectInfo {
            ...
            projectInfoDSL._scmConfig?.let { scmDSL ->
                scm {
                    ...
                    if (scmDSL._developerConnection.isNotBlank())
                        developerConnection = scmDSL._developerConnection
                }
            }
            ...
            projectInfoDSL._licenseConfig?.let { licenseDSL ->
                license {
                    ...
                    if (licenseDSL._distribution.isNotBlank())
                        distribution = licenseDSL._distribution
                }
            }
            ...
            projectInfoDSL._developerConfig?.let { developerDSL ->
                developer {
                    ...
                    if (developerDSL._organization.isNotBlank())
                        organization = developerDSL._organization
                    if (developerDSL._organizationUrl.isNotBlank())
                        organizationUrl = developerDSL._organizationUrl
                }
            }
        }
    }
    ...
    fun signing(configure: SigningDSL.() -> Unit) {
        ...
        configBuilder.signing {
            ...
            if (signingDSL._secretKeyRingFile.isNotBlank())
                secretKeyRingFile = signingDSL._secretKeyRingFile
        }
    }
    ...
    /**
     * Returns whether the user has explicitly configured any part of the plugin. Used to determine
     * if validation errors should be shown during configuration phase.
     */
...
class CredentialsDSL {
     ...
     *
     * It's recommended to use a token instead of your actual password. You can generate tokens in
     * your Sonatype account settings.
     */
    var password: String
        get() = _password
        set(value) {
 ...
 *
 * Supports auto-detection from git repositories, build files, and README files. Explicit
 * configuration takes precedence over auto-detected values.
 *
 * @since 0.0.5
 */
@CentralPublisherDsl
 ...
 *
 * Provides type-safe configuration for repository URLs and connections. Supports auto-detection
 * from git remotes when not explicitly configured.
 *
 * @since 0.0.5
 */
@CentralPublisherDsl
 ...
 *
 * Provides type-safe configuration for GPG key information required for artifact signing. Maven
 * Central requires all artifacts to be signed.
 *
 * @since 0.0.5
 */
@CentralPublisherDsl
class SigningDSL {
     ...
     *
     * This should be the last 8 characters of your GPG key ID. Example: "12345678"
     */
    var keyId: String
        get() = _keyId
        set(value) {
     ...
     *
     * Example: "~/.gnupg/secring.gpg" Note: Modern GPG versions store keys differently, so this may
     * not be needed.
     */
    var secretKeyRingFile: String
        get() = _secretKeyRingFile
        set(value) {
            _secretKeyRingFile = value
        }
}
 ...
 *
 * Provides type-safe configuration for publishing settings like auto-publish, aggregation, and
 * dry-run mode.
 *
 * @since 0.0.5
 */
...
class PublishingDSL {
     ...
     *
     * When true, artifacts are automatically promoted to public repositories. When false, you need
     * to manually promote them through the Sonatype UI. Default: false (safer for production)
     */
     ...
     *
     * When true, all artifacts are uploaded to a single staging repository. When false, each
     * artifact gets its own staging repository. Default: true (recommended)
     */
     ...
     *
     * When true, all operations are simulated without actual publishing. Useful for testing
     * configuration and validation. Default: false
     */
    var dryRun: Boolean
        get() = _dryRun ?: false
        set(value) {
