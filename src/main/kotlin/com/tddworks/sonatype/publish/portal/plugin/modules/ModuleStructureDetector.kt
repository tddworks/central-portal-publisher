...
class ModuleStructureDetector : AutoDetector {
    ...
    override fun detect(project: Project): com.tddworks.sonatype.publish.portal.plugin.autodetection.DetectionResult? {
        val projectDir = project.projectDir
        
        // Check if this is a valid Gradle project (either has build file or settings file for multi-module)
        ...
        detectedValues["modules.structure"] = DetectedValue(
            path = "modules.structure",
            value = if (moduleStructure.isMultiModule) "multi-module" else "single-module",
            source = if (moduleStructure.isMultiModule) "settings.gradle[.kts]" else "build.gradle[.kts]",
            confidence = Confidence.HIGH
        )

        if (moduleStructure.isMultiModule) {
            detectedValues["modules.count"] = DetectedValue(
                path = "modules.count", 
            ...
            detectedValues["modules.publishable"] = DetectedValue(
        ...
        // Create minimal config - most configuration will be handled by MultiModuleConfigurationManager
    ...
    private fun analyzeMultiModuleStructure(rootDir: File, settingsFile: File): ModuleStructure {
        ...
        val subModules = includedModules.mapNotNull { modulePath ->
    ...
    private fun extractModuleDependencies(buildFile: File): List<String> {
        ...
        return projectDependencyRegex.findAll(content)
...
data class DetectionResult(
    ...
    fun toStandardDetectionResult(): com.tddworks.sonatype.publish.portal.plugin.autodetection.DetectionResult {
        return com.tddworks.sonatype.publish.portal.plugin.autodetection.DetectionResult(
            config = config,
            detectedValues = detectedValues,
            warnings = warnings
