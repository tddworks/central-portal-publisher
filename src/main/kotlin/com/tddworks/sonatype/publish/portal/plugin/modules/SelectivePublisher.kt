...
class SelectivePublisher {
    ...
    fun selectModules(
        ...
        config: ModuleSelectionConfig,
    ): List<GradleModule> {
        // In multi-module projects, exclude root module if it's not publishable by default
        var candidates =
            if (moduleStructure.isMultiModule && !moduleStructure.rootModule.isPublishable) {
        ...
        if (config.includePatterns.isNotEmpty()) {
            candidates =
                candidates.filter { module ->
                    config.includePatterns.any { pattern -> matchesPattern(module.name, pattern) }
                }
        }
        ...
        if (config.excludePatterns.isNotEmpty()) {
            candidates =
                candidates.filter { module ->
                    config.excludePatterns.none { pattern -> matchesPattern(module.name, pattern) }
                }
        }
        ...
        if (config.selectedGroups.isNotEmpty()) {
            val groupedModules =
                config.selectedGroups.flatMap { groupName ->
                    config.groups[groupName] ?: emptyList()
                }
            candidates = candidates.filter { module -> groupedModules.contains(module.name) }
        }

        // Apply custom filters
        config.filters.forEach { filter -> candidates = filter.apply(candidates) }
...
sealed class ModuleFilter {
    ...
    data class HasDependencies(private val hasProjectDependencies: Boolean) : ModuleFilter() {
        override fun apply(modules: List<GradleModule>): List<GradleModule> {
    ...
    data class PathPattern(private val patterns: List<String>) : ModuleFilter() {
        override fun apply(modules: List<GradleModule>): List<GradleModule> {
            return modules.filter { module ->
                patterns.any { pattern -> matchesPattern(module.path, pattern) }
            }
        }
    ...
    data class DirectoryPattern(private val patterns: List<String>) : ModuleFilter() {
        override fun apply(modules: List<GradleModule>): List<GradleModule> {
            return modules.filter { module ->
                ...
                patterns.any { pattern -> matchesPattern(relativePath, pattern) }
            }
        }
