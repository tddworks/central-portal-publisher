...
interface AutoDetector {
    ...
    /**
     * Detect configuration values and return a partial configuration.
     * Returns null if this detector cannot provide any useful information.
     */
...
data class DetectedValue(
    val path: String,        // e.g. "projectInfo.name"
    val value: String,       // The detected value
    val source: String,      // e.g. "build.gradle.kts", ".git/config"
...
enum class Confidence {
    ...
    HIGH,
    /** Medium confidence - likely correct but might need verification (e.g., inferred from directory name) */
    MEDIUM,
...
class AutoDetectionManager(
    private val detectors: List<AutoDetector>
) {
    ...
    fun detectConfiguration(project: Project): AutoDetectionSummary {
        ...
        detectors.filter { it.enabledByDefault }.forEach { detector ->
            try {
                detector.detect(project)?.let { result ->
                    ...
                    result.detectedValues.forEach { (path, detectedValue) ->
                        ...
                        if (existing == null || detectedValue.confidence.ordinal < existing.confidence.ordinal) {
        ...
        val finalConfig = results.fold(
            com.tddworks.sonatype.publish.portal.plugin.config.CentralPublisherConfigBuilder().build()
        ) { acc, result ->
...
data class AutoDetectionSummary(
    ...
    val hasDetectedValues: Boolean get() = detectedValues.isNotEmpty()
