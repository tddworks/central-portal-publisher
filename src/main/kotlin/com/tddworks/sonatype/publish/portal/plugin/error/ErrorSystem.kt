...
/**
 * Comprehensive error code system for the Central Portal Publisher plugin. Provides structured
...
class PublishingErrorSystem {
    ...
    fun getSuggestionsFor(error: PublishingError): List<FixSuggestion> {
        return when (error.code) {
            ...
            ErrorCode.INVALID_POM ->
                listOf(
                    FixSuggestion(
                        type = FixSuggestionType.VALIDATION,
                        description =
                            "Check required POM fields (name, description, url, licenses, developers, scm)",
                        priority = 1,
                    ),
                    FixSuggestion(
                        type = FixSuggestionType.CONFIGURATION,
            ...
            ErrorCode.MISSING_DESCRIPTION ->
                listOf(
                    FixSuggestion(
                        ...
                        command =
                            "echo 'POM_DESCRIPTION=Your project description' >> gradle.properties",
                        priority = 1,
                    )
                )
            ErrorCode.UNKNOWN ->
            ...
            error.context?.let { ctx ->
                buildString {
                    ...
                    if (ctx.additionalInfo.isNotEmpty()) {
                        ctx.additionalInfo.forEach { (key, value) -> append("  $key: $value\n") }
                    }
                }
            } ?: ""
    ...
    fun createErrorReport(errors: List<PublishingError>): ErrorReport {
        ...
        val formattedOutput = buildString {
            ...
            ErrorCategory.values()
                .filter { it != ErrorCategory.UNKNOWN }
                .forEach { category ->
