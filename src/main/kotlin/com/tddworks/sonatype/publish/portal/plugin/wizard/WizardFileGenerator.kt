...
class DefaultWizardFileGenerator : WizardFileGenerator {
    ...
    override fun generateFiles(
        ...
        finalConfig: CentralPublisherConfig,
    ): List<String> {
    ...
    private fun hasGlobalCredentials(): Boolean {
        ...
        val hasUsername =
            content.contains("SONATYPE_USERNAME=") &&
                content
                    .lines()
                    ...
                    ?.isNotBlank() == true
        val hasPassword =
            content.contains("SONATYPE_PASSWORD=") &&
                content
                    .lines()
        ...
        val hasSigningPassword =
            content.contains("SIGNING_PASSWORD=") &&
                content
                    .lines()
    ...
    private fun generateGradleProperties(
        ...
        val content = buildString {
            ...
            appendLine(
                "# Consider using environment variables or global gradle.properties instead."
            )
            appendLine()
    ...
    private fun generateBuildFile(context: WizardContext, finalConfig: CentralPublisherConfig) {
        ...
        val projectUrl =
            finalConfig.projectInfo.url.takeIf { it.isNotBlank() }
        ...
        val developerId =
            firstDeveloper?.id?.takeIf { it.isNotBlank() }
                ?: firstDeveloper?.email?.substringBefore("@")
                ?: detectedDeveloper?.email?.substringBefore("@")
                ?: "yourid"
        val developerName =
            firstDeveloper?.name?.takeIf { it.isNotBlank() }
                ?: detectedDeveloper?.name
                ?: "Your Name"
        val developerEmail =
            firstDeveloper?.email?.takeIf { it.isNotBlank() }
        ...
        val centralPublisherBlock = buildString {
            ...
            val projectName =
                finalConfig.projectInfo.name.takeIf { it.isNotBlank() }
                    ?: context.detectedInfo?.projectName
                    ?: context.project.name
            val projectDescription =
                finalConfig.projectInfo.description.takeIf { it.isNotBlank() }
    ...
    private fun updateExistingBuildFile(buildFile: File, centralPublisherBlock: String) {
        ...
        val updatedContent =
            if (existingContent.contains("centralPublisher {")) {
    ...
    private fun addPluginAndBlock(content: String, centralPublisherBlock: String): String {
        ...
        return if (pluginsMatch != null) {
            ...
            content.replace(pluginsMatch.value, "plugins {\n$newPluginsContent\n}") +
                "\n\n$centralPublisherBlock"
        } else {
