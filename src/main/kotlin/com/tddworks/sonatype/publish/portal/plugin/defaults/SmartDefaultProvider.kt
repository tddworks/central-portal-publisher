 ...
 *
 * Smart defaults are applied when neither DSL configuration nor auto-detection can provide a value.
 * They are context-aware and adapt based on project type, structure, and other environmental
 * factors.
 */
interface SmartDefaultProvider {
     ...
     *
     * Only returns values that this provider can confidently default. Returns partial
     * configuration - other providers may fill in missing values.
     *
...
class SmartDefaultManager(private val providers: List<SmartDefaultProvider> = emptyList()) {
    ...
    constructor(
        ...
        listOf(GenericProjectDefaultProvider())
     ...
     *
     * Providers are applied in priority order (highest first). Each provider can override values
     * set by lower-priority providers.
     *
     * @param project The Gradle project
    ...
    fun applySmartDefaults(
        project: Project,
        baseConfig: CentralPublisherConfig,
    ): CentralPublisherConfig {
    ...
    private fun mergeScm(base: ScmConfig, defaults: ScmConfig): ScmConfig {
        return base.copy(
            url = base.url.ifBlank { defaults.url },
    ...
    private fun mergeLicense(base: LicenseConfig, defaults: LicenseConfig): LicenseConfig {
        return base.copy(
            name = base.name.ifBlank { defaults.name },
            url = base.url.ifBlank { defaults.url },
    ...
    private fun mergeSigning(base: SigningConfig, defaults: SigningConfig): SigningConfig {
        return base.copy(
    ...
    private fun mergePublishing(
        ...
        return base.copy(
            ...
            aggregation =
                if (base.aggregation != defaults.aggregation) base.aggregation
                else defaults.aggregation,
