...
interface ZipPublicationTaskFactory {
    ...
    fun createZipAggregationPublicationsTask(project: Project): TaskProvider<Zip>
}
...
class SonatypeZipPublicationTaskFactory : ZipPublicationTaskFactory {
    ...
    override fun createZipTask(
        ...
        // get the sonatype build repository directory for task e.g
            ...
            dependsOnTask
                ...
                .get(
                    SonatypePublishPublicationToMavenRepositoryTaskFactory
                        .SONATYPE_BUILD_REPOSITORY_DIRECTORY
                ) as File

        println(
            "creating zip task for project ${project.name} with publicationName: $publicationName"
        )

        val zipTaskProvider =
            project.tasks.register(
                taskName(publicationName.replaceFirstChar { it.uppercase() }),
                Zip::class.java,
            ) {
    ...
    override fun createZipAggregationPublicationsTask(project: Project): TaskProvider<Zip> {
        return project.tasks.register(
            ...
            from(
                project.configurations.getByName(ZIP_CONFIGURATION_CONSUMER).elements?.map { bundle
                    ->
                    logger.quiet("Sonatype Portal Publisher plugin found publish bundle: $bundle")
                    check(bundle.isNotEmpty()) { "No bundle found for project: $path" }
                    bundle.map { project.zipTree(it) }
                }
            )
