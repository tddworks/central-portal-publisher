...
class GitInfoDetector : AutoDetector {
    ...
    override fun detect(project: Project): DetectionResult? {
        val gitDir = findGitDirectory(project.projectDir) ?: return null

        val detectedValues = mutableMapOf<String, DetectedValue>()
        val warnings = mutableListOf<String>()
        val configBuilder =
            CentralPublisherConfigBuilder().withSource(ConfigurationSource.AUTO_DETECTED)
        ...
        if (remoteUrl != null) {
            ...
            configBuilder.projectInfo {
                if (httpsUrl != null) {
                    url = httpsUrl
                    detectedValues["projectInfo.url"] =
                        DetectedValue("projectInfo.url", httpsUrl, ".git/config", Confidence.HIGH)
                }

                scm {
                    if (httpsUrl != null) {
                        url = httpsUrl
                        detectedValues["projectInfo.scm.url"] =
                            DetectedValue(
                                "projectInfo.scm.url",
                                httpsUrl,
                                ".git/config",
                    ...
                    if (scmConnection != null) {
                        ...
                        detectedValues["projectInfo.scm.connection"] =
                            DetectedValue(
                    ...
                    if (devConnection != null) {
                        ...
                        detectedValues["projectInfo.scm.developerConnection"] =
                            DetectedValue(
        ...
        if (developerInfo != null) {
            ...
            detectedValues["projectInfo.developer.name"] =
                DetectedValue(
            ...
            detectedValues["projectInfo.developer.email"] =
                DetectedValue(
        ...
        val httpsUrl =
            when {
                gitUrl.startsWith("git@github.com:") -> {
                    // Convert SSH to HTTPS: git@github.com:user/repo.git ->
                    // https://github.com/user/repo
                    "https://github.com/" + gitUrl.substring(15).removeSuffix(".git")
                }
                gitUrl.startsWith("git@gitlab.com:") -> {
                    // Convert SSH to HTTPS: git@gitlab.com:user/repo.git ->
    ...
    private fun runGitCommand(workingDir: File, vararg args: String): String? {
        return try {
            val process =
                ProcessBuilder("git", *args).directory(workingDir).redirectErrorStream(true).start()
