    ...
    fun execute() {
        ...
        try {
            ...
            if (result.violations.isEmpty()) {
                ...
                project.logger.quiet("   • Credentials: ${if (config.credentials.username.isNotEmpty()) "✓ Configured" else "⚠ Missing"}")
                project.logger.quiet("   • Signing: ${if (config.signing.keyId.isNotEmpty() || config.signing.secretKeyRingFile.isNotEmpty()) "✓ Configured" else "⚠ Missing"}")
                project.logger.quiet("   • License: ${config.projectInfo.license.name.ifEmpty { "⚠ Not specified" }}")
                project.logger.quiet("💡 Ready to publish! Run './gradlew publishToCentral' when ready.")
            } else {
                ...
                throw GradleException("Configuration validation failed. Please fix the issues above.")
            }
