...
/** Service interface for user operations. This is the public API that clients will use. */
...
class InMemoryUserService : UserService {
    ...
    override suspend fun createUser(name: String, email: String): Result<User> {
        return try {
            val user = User(id = "user-${nextId++}", name = name, email = email)
    ...
    override suspend fun getUser(userId: String): Result<User> {
        return users[userId]?.let { Result.Success(it) } ?: Result.Error("User not found: $userId")
    }
