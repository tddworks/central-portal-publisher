...
/** Test data generators for unit testing. This module is internal and not published. */
object TestData {
    ...
    fun createTestUsers(count: Int): List<User> {
        return (1..count).map { i ->
            User(id = "test-user-$i", name = "Test User $i", email = "test$i@example.com")
        }
    }
    ...
    fun generateEmails(count: Int): List<String> {
        ...
        return (1..count).map { i -> "user$i@${domains.random()}" }
    }
}
